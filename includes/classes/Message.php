  ...
  public function getMostRecentUser()
  {
    ...
    public function  sendMessage($user_to, $body, $date)
    {
        if($body != ""){
            ...
            if(!$query){
                die("failed to send message".mysqli_errno($this->conn));
            }
        }
    }
    ...
    public function getMessages($otherUser)
    {
        ...
        while ($row = mysqli_fetch_array($get_messages_query)) {
            ...
            $div_top = ($user_to == $userLoggedIn) ? "<p class='message' id='green'>" : "<p class='message' id='blue'>";
            //$button = "<span class='deleteButton' onclick='deleteMessage($id, this)'>X</span>";
            $data = $data . $div_top . $body . "</p><br><br>";
        }
        return $data;
    }
    ...
    public function getConvos(){
        ...
        $query = mysqli_query($this->conn,"SELECT * FROM messages WHERE user_to='$userLoggedIn' OR user_from='$userLoggedIn' ORDER BY id DESC");
        ...
        foreach($convos as $username){
            ...
            <span class='user_found_name'>".$user_found_obj->getName(). "</span><span class='timestamp_smaller' id='grey'>&nbsp".$latest_message_details[2]."</span>
    ...
    public function getConvosDropdown($data,$limit){
        ...
        $num_iterations = 0;//number of messages retrieved
        $count= 0;
        foreach ($convos as $username) {
